...
struct vec2 {
	...
	friend constexpr vec2 operator+(float s, const vec2& vec) { 
		return vec + s; 
	}
...
struct vec3 {
	...
	friend constexpr vec3 operator+(float s, const vec3& vec) { 
		return vec + s; 
	}
...
struct vec4 {
	...
	friend constexpr vec4 operator+(float s, const vec4& vec) { 
		return vec + s; 
	}
	...
	friend constexpr vec4 operator-(float s, const vec4& vec) {
		return vec4(s - vec.x, s - vec.y, s - vec.z, s - vec.w); 
	}
